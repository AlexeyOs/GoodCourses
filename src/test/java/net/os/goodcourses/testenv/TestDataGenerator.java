...
public class TestDataGenerator {
	...
	private static final String[] CITIES = { "Moscow", "Peter", "Krasnodar" };
	...
	private static final List<String> platforms = new ArrayList<>(
			Arrays.asList("job4j", "javaRush", "udemy", "coursera.org", "sso.openedu.ru", "stepik.org", "codecademy.com")
	);
	private static final List<String> authors = new ArrayList<>(
			Arrays.asList("Superman","Бэтмен","Человек Паук", "Шакро Молодой", "Япончик", "Хасан", "Хотабыч")
	);
	private static final List<String> subjectOfStudy = new ArrayList<>(
			Arrays.asList("Java", "Java", "Java Spring", "Machine Learning", "C#", "Kotlin", "SQL")
	);
	private static final List<String> links = new ArrayList<>(Arrays.asList("https://job4j.ru/courses/java_with_zero_to_job.html",
			"https://javarush.ru/",
	...
	private static void insertProfileData(Connection c, Profile profile, ProfileConfig profileConfig) throws SQLException, IOException {
		...
		if (r.nextBoolean()) {
			ps.setString(11, getInfo());
		} else {
			ps.setNull(11, Types.VARCHAR);
		}
		ps.setString(12, PASSWORD_HASH);

		ps.setTimestamp(13, new Timestamp(System.currentTimeMillis()));

		if (r.nextBoolean()) {
			ps.setString(14, (profile.firstName + "-" + profile.lastName).toLowerCase());
		} else {
			ps.setNull(14, Types.VARCHAR);
		}
		if (r.nextBoolean()) {
			ps.setString(15, "https://vk.com/" + (profile.firstName + "-" + profile.lastName).toLowerCase());
		} else {
			ps.setNull(15, Types.VARCHAR);
		}
		if (r.nextBoolean()) {
			ps.setString(16, "https://facebook.com/" + (profile.firstName + "-" + profile.lastName).toLowerCase());
		} else {
			ps.setNull(16, Types.VARCHAR);
		}
		if (r.nextBoolean()) {
			ps.setString(17, "https://linkedin.com/" + (profile.firstName + "-" + profile.lastName).toLowerCase());
		} else {
			ps.setNull(17, Types.VARCHAR);
		}
		if (r.nextBoolean()) {
			ps.setString(18, "https://github.com/" + (profile.firstName + "-" + profile.lastName).toLowerCase());
		} else {
			ps.setNull(18, Types.VARCHAR);
		}
		if (r.nextBoolean()) {
			ps.setString(19, "https://stackoverflow.com/" + (profile.firstName + "-" + profile.lastName).toLowerCase());
		} else {
